...
for model in models:
    ...
    acc.append([model.score(X_test, y_test), model.__str__()[:6]])
print(acc, max(acc), acc.index(max(acc)))

# Итоговая модель
model = XGBClassifier(gamma=1.2, max_depth=6, min_child_weight=6, n_estimators=50, objective='binary:logistic')
model.fit(X,y)
